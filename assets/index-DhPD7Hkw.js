  ...
  <div id="modal-box" class="modal-box">
    ...
    <ul class="modal-info-list" id="modal-info-list">
      ...
      <li class="modal-info-item" id="modal-where">
        ...
        <p class="modal-text">
          ${R}, ${q}<span class="br-space"></span>${w}
        </p>
      </li>
...
`,d.classList.add("active"),d.querySelector("#close-modal-butt").addEventListener("click",()=>d.classList.remove("active")),d.addEventListener("click",p=>{p.target===d&&d.classList.remove("active")}),document.querySelector("#more-author-butt").addEventListener("click",p=>{const C=p.target.dataset.url;C&&C!=="#"?window.open(C,"_blank","noopener,noreferrer"):console.warn("URL not provided")})}let r=0,c;function P(){r=0}function z(e){r=e}function h(e){const n=document.createElement("button");return n.classList.add("pagination-unit"),n.textContent=e+1,e===r&&n.classList.add("active"),n}function G(e){c=e,A.innerHTML="";let n=[];function a(){const t=document.createElement("span");t.textContent="...",t.classList.add("pagination-dots"),n.push(t)}if(c<=5)for(let t=0;t<c;t++)n.push(h(t));else{n.push(h(0));let t=Math.max(1,r-2),o=Math.min(c-2,r+2);r<2&&(t=1,o=4),r>c-4&&(t=c-5,o=c-2),t>1&&a();for(let s=t;s<=o;s++)n.push(h(s));o<c-2&&a(),n.push(h(c-1))}n.map(t=>A.insertAdjacentElement("beforeend",t))}function S(e,n){let a;return(...t)=>{clearTimeout(a),a=setTimeout(()=>e(...t),n)}}async function F(){const e=await fetch("https://ipapi.co/json/");return e.ok?(await e.json()).country:null}function $(){const e=window.innerWidth;return e>=1280?20:e>=768?21:20}let u,v=[],b="",l=$();U();async function f(e,n,a,t){z(e),E.classList.add("active"),M([]);const o=await O(e,n,a,t);if(!o?._embedded?.events){if(t!=="US"){t="US",B.textContent="US",await f(e,n,a,t);return}v=[],M(v),S(E.classList.remove("active"),500);return}S(E.classList.remove("active"),500),v=o._embedded.events;const s=o?.page?.totalPages||1;G(Math.min(s,50)),S(M(v),300),U()}H?.addEventListener("submit",e=>{e.preventDefault()});H?.addEventListener("input",e=>{e.preventDefault(),e.target.closest("input")});L.addEventListener("click",e=>j(v,e));A.addEventListener("click",async e=>{if(e.target.classList.contains("pagination-unit")){const n=parseInt(e.target.textContent)-1;await f(n,b,l,u)}});g.addEventListener("click",async e=>{e.preventDefault();const n=e.target;if(n.tagName!=="A")return;const a=n.textContent.trim();u=a,B.textContent=a,await f(r,b,l,u)});window.addEventListener("resize",()=>{const e=$();e!==l&&(l=e,P(),f(r,b,l,u))});async function W(){u=await F(),f(r,"",l,u)}window.addEventListener("resize",()=>{const e=$();e!==l&&(l=e,P(),f(r,b,l,u))});W();
