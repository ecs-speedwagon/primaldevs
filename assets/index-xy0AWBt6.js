(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))t(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function s(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function t(o){if(o.ep)return;o.ep=!0;const r=s(o);fetch(o.href,r)}})();const R="q6wGVb9Aq0qhPo2kRkaUMu7npvf9A2ZE",I="https://app.ticketmaster.com/discovery/v2";async function N(e,n,s,t){try{const o=`${I}/events.json?apikey=${R}&size=${s}&page=${e}&keyword=${n||""}&countryCode=${t}`,r=await fetch(o);if(!r.ok)throw new Error(`Request failed: ${r.status}`);const i=await r.json();if(i?._embedded?.events)return i}catch(o){console.error("error get",o.message)}}const g=document.querySelector(".events-ul"),b=document.querySelector(".dropdown-menu"),x=document.querySelector(".dropdown-toggle"),E=document.querySelector(".pagination-box"),q=document.querySelector("#searching-input"),u=document.querySelector("#modal-vignette");Number.parseInt(document.documentElement.clientWidth);const A=document.querySelector(".loader");function $(e){let n=0;if(n=e,!g)return;g.innerHTML="";const s=n.map((t,o)=>{const r=t.images?.find(w=>w.ratio==="3_2")?.url||"",i=t.dates?.start?.localDate||"No date info",y=t._embedded?.venues?.[0]?.name||"We don't have enough info",f=typeof t.name=="string"&&t.name.trim()!==""?t.name:"Untitled event";return`
    ...
    `}).join("");g.insertAdjacentHTML("beforeend",s)}const D=["US","AD","AI","AR","AU","AT","AZ","BS","BH","BB","BE","BM","BR","BG","CA","CL","CN","CO","CR","HR","CY","CZ","DK","DO","EC","EE","FO","FI","FR","GE","DE","GH","GI","GB","GR","HK","HU","IS","IN","IE","IL","IT","JM","JP","KR","LV","LB","LT","LU","MY","MT","MX","MC","ME","MA","NL","AN","NZ","ND","NO","PE","PL","PT","RO","RU","LC","SA","RS","SG","SK","SI","ZA","ES","SE","CH","TW","TH","TT","TR","UA","AE","UY","VE"];function T(){if(!b)return;b.innerHTML="";const e=D.map(n=>`
        <li>
          <a class="dropdown-item" href="#">${n}</a>
        </li>
      `).join("");b.insertAdjacentHTML("beforeend",e)}function O(e,n){const s=n.target.closest(".event-item");if(!s)return;const t=e[s.dataset.id],o=t.url||"#",r=t.images?.find(L=>L.ratio==="3_2")?.url||"",i=t.dates?.start?.localDate||"No date info",H=t.dates?.start?.localTime.slice(0,5)||"Unknown time",y=t.dates?.timezone||"Local time",f=t._embedded?.venues?.[0],w=f?.name||"We don't have enough info",B=f?.city?.name||"",k=f?.country?.name||"",S=typeof t.description=="string"&&t.description.trim()!==""?t.description.slice(0,140)+(t.description.length>140?"…":""):!1,P=t._embedded?.attractions?.[0]?.name||"Unknown performer",Z=typeof t.name=="string"&&t.name.trim()!==""?t.name:"Untitled event";u.innerHTML=`<div id="modal-box" class="modal-box">
...
class="modal-info-item" id="modal-when">
    ...
    </div>`,u.classList.add("active"),u.querySelector("#close-modal-butt").addEventListener("click",()=>u.classList.remove("active")),u.addEventListener("click",L=>{L.target===u&&u.classList.remove("active")}),document.querySelector("#more-author-butt").addEventListener("click",L=>{const C=L.target.dataset.url;C&&C!=="#"?window.open(C,"_blank","noopener,noreferrer"):console.warn("URL not provided")})}let a=0,l;function U(){a=0}function _(e){a=e}function v(e){const n=document.createElement("button");return n.classList.add("pagination-unit"),n.textContent=e+1,e===a&&n.classList.add("active"),n}function j(e){l=e,E.innerHTML="";let n=[];function s(){const t=document.createElement("span");t.textContent="...",t.classList.add("pagination-dots"),n.push(t)}if(l<=5)for(let t=0;t<l;t++)n.push(v(t));else{n.push(v(0));let t=Math.max(1,a-2),o=Math.min(l-2,a+2);a<2&&(t=1,o=4),a>l-4&&(t=l-5,o=l-2),t>1&&s();for(let r=t;r<=o;r++)n.push(v(r));o<l-2&&s(),n.push(v(l-1))}n.map(t=>E.insertAdjacentElement("beforeend",t))}function z(e,n){let s;return(...t)=>{clearTimeout(s),s=setTimeout(()=>e(...t),n)}}async function G(){const e=await fetch("https://ipapi.co/json/");return e.ok?(await e.json()).country:null}function M(){const e=window.innerWidth;return e>=1280?20:e>=768?21:20}let d,h=[],p="",c=M();T();async function m(e,n,s,t){_(e),A.classList.add("active");const o=await N(e,n,s,t);if(!o?._embedded?.events){if(t!=="US"){t="US",x.textContent="US",await m(e,n,s,t);return}h=[],$(h);return}A.classList.remove("active"),h=o._embedded.events;const r=o?.page?.totalPages||1;j(Math.min(r,50)),$(h),T()}q?.addEventListener("input",z(e=>{p=e.target.value,m(a,p,c,d)},300));g.addEventListener("click",e=>O(h,e));E.addEventListener("click",async e=>{if(e.target.classList.contains("pagination-unit")){const n=parseInt(e.target.textContent)-1;await m(n,p,c,d)}});b.addEventListener("click",async e=>{e.preventDefault();const n=e.target;if(n.tagName!=="A")return;const s=n.textContent.trim();d=s,x.textContent=s,await m(a,p,c,d)});window.addEventListener("resize",()=>{const e=M();e!==c&&(c=e,U(),m(a,p,c,d))});async function W(){d=await G(),m(a,"",c,d)}window.addEventListener("resize",()=>{const e=M();e!==c&&(c=e,U(),m(a,p,c,d))});W();
